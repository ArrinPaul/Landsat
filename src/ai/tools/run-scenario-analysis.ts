...
export const runScenarioAnalysis = ai.defineTool(
  ...
  async (input) => {
    ...
    const scenarioPrompt = `You are an expert environmental scientist and agronomist.
    ...
    // We'll use another LLM call to structure the response, ensuring consistency.
    const structurePrompt = ai.definePrompt({
        name: 'structureScenarioPrompt',
    ...
    const { output } = await structurePrompt({});

    if (!output) {
      throw new Error("Failed to structure scenario analysis output.");
    }
